...
def load_qqp(dataset="qqp",
             split="train",
             sample=None,
             paws_num=0) -> List[TextPairExample]:
  if split in ["train", "dev", "test"]:
    if dataset == "qqp":
      filename = join(config.QQP_SOURCE, "{}.tsv".format(split))
    elif dataset == "qqp_paws":
      tsvname = "{}_qqp_paws_{}.tsv".format(
          split, paws_num) if split == "train" else "test_paws.tsv"
  ...
  for line in lines:
    ...
    if len(line) != 6:
      continue
    out.append(TextPairExample(line[0], line[3], line[4], int(line[-1])))
...
def load_paws(is_train=False) -> List[TextPairExample]:
  ...
  for line in lines:
    line = line.split("\t")
    out.append(TextPairExample(line[0], line[1], line[2], int(line[-1])))
  return out
...
def main():
  ...
  train_examples = None
  if args.do_train:
    train_examples = load_qqp(
        dataset=args.qqp_dataset,
        split="train",
        paws_num=args.paws_num,
  ...
  if args.do_eval:
    ...
    eval_datasets += [("paws_test",
                       load_qqp(dataset=args.qqp_dataset, split="test"))]
  ...
  for ix, (name, eval_examples) in enumerate(eval_datasets):
    ...
    result["duplicate_acc"] = ((preds == all_label_ids).astype(np.int32) *
                               all_label_ids).sum() / all_label_ids.sum()
