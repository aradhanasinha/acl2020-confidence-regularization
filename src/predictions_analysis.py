...
def visualize_predictions(all_probs: np.ndarray, all_labels: np.ndarray,
  ...
  all_bins_stat = [[0, 0, 0.] for _ in range(num_bins)
                  ]  # cor_num, num, conf_accum
  ...
  plt.bar(
      y_pos, acc_fracts, align='center', alpha=0.5, label='correctly predicted')
...
def visualize_penultimate(all_labels: np.ndarray,
                          all_is_correct: np.ndarray,
                          all_penultimates: np.ndarray,
                          all_dataset_ids: np.ndarray,
                          plot_file: str = None,
                          sample=-1):
  if sample > 0:
  ...
  for dat_ix in np.unique(all_dataset_ids):
    for label in np.unique(all_labels):
      ...
      for cur_label, cur_is_correct, cur_dataset_id, cur_penultimate in zip(
          all_labels, all_is_correct, all_dataset_ids, all_penultimates):
      ...
      if len(cur_incor_penultimates) > 0:
        ...
        if dat_ix == 0:
          plt.scatter(
              cur_incor_penultimates[:, 0],
              cur_incor_penultimates[:, 1],
              alpha=0.8,
              marker='x',
              color=color_types[label],
              s=size)
        else:
          plt.scatter(
              cur_incor_penultimates[:, 0],
      ...
      plt.scatter(
          cur_penultimates[:, 0],
          cur_penultimates[:, 1],
          label=dataset_str + '_' + label_str,
          alpha=0.5,
          marker=marker_types[dat_ix],
          color=color_types[label],
          s=size)
